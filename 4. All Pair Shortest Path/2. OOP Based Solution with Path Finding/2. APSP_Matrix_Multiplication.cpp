...
public:
    ...
    vector<vector<double>> apsp_by_matrix_multiplication();
...
vector<vector<double>> Graph::apsp_by_matrix_multiplication() {
    vector<vector<double>> distanceMatrix = this->adjMatrix;
    for(int l=0; l<V-2; l++){
        for(int i=0; i<V; i++){
            for(int j=0; j<V; j++){
                for(int k=0; k<V; k++){
                    if(distanceMatrix[i][k] == numeric_limits<double>::max() || distanceMatrix[k][j] == numeric_limits<double>::max())
                        continue;
                    if(distanceMatrix[i][k] + adjMatrix[k][j]<distanceMatrix[i][j]){
                        distanceMatrix[i][j] = distanceMatrix[i][k] + adjMatrix[k][j];
...
int main() {
    ...
    distanceMatrix = graphInstance.apsp_by_matrix_multiplication();
